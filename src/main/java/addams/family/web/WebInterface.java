...
import addams.family.web.thingoncloud.ThingOnCloudThread;
...
//TODO reload on properties change
	...
	private ThingOnCloudThread toct;
	...
	public WebInterface() {
		...
		senst.start();
		toct = new ThingOnCloudThread(prop.getTOCProperty());
		toct.start();
	}
	...
	public void init() {
        ...
        post("/extLight/:value", (request, response) -> {
            Integer val = Integer.parseInt(request.params(":value"));
            if(val >= 0 && val < 256) {
                db.setExternalBrightness("E1", val);
                db.setExternalBrightness("E2", val);
                db.setExternalBrightness("E3", val);
                db.setExternalBrightness("E4", val);
            }
            return "eBrightness ALL: " + val;
        });
